export default function HelpButton({ text, image, onClick }: { text: string; image: string; onClick: () => void }) {
	return (
		<button
			className={`hover:scale-[98%] flex flex-col items-center border-[1px] border-solid border-[var(--border-color)] rounded-[var(--rounded)] w-full h-[100%] justify-center p-[20px] gap-[20px] bg-[var(--bg-muted)]`}
			onClick={onClick}
		>
